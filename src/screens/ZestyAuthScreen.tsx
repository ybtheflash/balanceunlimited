...
export default function ZestyAuthScreen({ onBack }: ZestyAuthScreenProps) {
  ...
  return (
    <KeyboardAvoidingView
      className={`${isLight ? 'bg-zinc-50' : 'bg-zinc-950'} flex-1  items-center`}
        ...
        <ScrollView className="flex-1 w-full" contentContainerStyle={{ paddingBottom: 100 }} showsVerticalScrollIndicator={false}>
          ...
          <View className="px-5 pt-14 pb-6">
            <View className="flex-row items-center gap-3 mb-6">
              <TouchableOpacity
                onPress={onBack}
                className={`${isLight ? 'bg-white' : 'bg-zinc-900'} w-10 h-10  rounded-xl items-center justify-center border border-zinc-800`}
              >
              ...
              <View className="flex-row items-center gap-2">
                ...
                <View>
                  <Text className={`${isLight ? 'text-zinc-900' : 'text-white'}  text-lg font-bold tracking-tight`}>ZestyAuth</Text>
                  <Text className={`${isLight ? 'text-zinc-600' : 'text-zinc-500'}  text-xs`}>Two-Factor Authentication</Text>
                </View>
              </View>
            </View>
            ...
            <View className={`p-4 rounded-2xl border mb-6 ${is2FAEnabled ? "bg-emerald-500/5 border-emerald-500/20" : "bg-amber-500/5 border-amber-500/20"}`}>
              <View className="flex-row items-center gap-3">
                ...
                <View className="flex-1">
                  ...
                  <Text className={`${isLight ? 'text-zinc-600' : 'text-zinc-500'}  text-xs mt-0.5`}>
                    {is2FAEnabled
            ...
            {setupMode ? (
              <View className="gap-5">
                <Text className={`${isLight ? 'text-zinc-900' : 'text-white'}  text-lg font-bold`}>Setup ZestyAuth</Text>
                
                {/* Step 1: Open Authenticator */}
                <View className={`${isLight ? 'bg-white' : 'bg-zinc-900'}  p-4 rounded-2xl border border-zinc-800`}>
                  <View className="flex-row items-center gap-2 mb-3">
                    <View className="w-7 h-7 bg-blue-600 rounded-lg items-center justify-center">
                      <Text className={`${isLight ? 'text-zinc-900' : 'text-white'}  font-bold text-xs`}>1</Text>
                    </View>
                    <Text className={`${isLight ? 'text-zinc-900' : 'text-white'}  font-semibold text-sm`}>Open Your Authenticator App</Text>
                  </View>
                  <Text className={`${isLight ? 'text-zinc-500' : 'text-zinc-400'}  text-xs leading-5`}>
                ...
                <View className={`${isLight ? 'bg-white' : 'bg-zinc-900'}  p-4 rounded-2xl border border-zinc-800`}>
                  <View className="flex-row items-center gap-2 mb-3">
                    <View className="w-7 h-7 bg-blue-600 rounded-lg items-center justify-center">
                      <Text className={`${isLight ? 'text-zinc-900' : 'text-white'}  font-bold text-xs`}>2</Text>
                    </View>
                    <Text className={`${isLight ? 'text-zinc-900' : 'text-white'}  font-semibold text-sm`}>Add Account Manually</Text>
                  </View>
                  <Text className={`${isLight ? 'text-zinc-500' : 'text-zinc-400'}  text-xs leading-5 mb-3`}>
                    In your authenticator app, choose "Add account" → "Enter setup key manually" and enter the details below:
                  </Text>

                  <View className={`${isLight ? 'bg-zinc-50' : 'bg-zinc-950'}  p-3 rounded-xl border border-zinc-800 mb-2`}>
                    <Text className={`${isLight ? 'text-zinc-600' : 'text-zinc-500'}  text-[10px] uppercase tracking-wider font-bold mb-1`}>Account Name</Text>
                    <Text className={`${isLight ? 'text-zinc-900' : 'text-white'}  text-sm font-medium`}>Balance Unlimited ({user?.email})</Text>
                  </View>

                  <View className={`${isLight ? 'bg-zinc-50' : 'bg-zinc-950'}  p-3 rounded-xl border border-zinc-800`}>
                    <Text className={`${isLight ? 'text-zinc-600' : 'text-zinc-500'}  text-[10px] uppercase tracking-wider font-bold mb-1`}>Secret Key</Text>
                  ...
                  {/* QR Code */}
                  <View className={`${isLight ? 'bg-zinc-50' : 'bg-zinc-950'}  p-4 rounded-xl border border-zinc-800 mt-2 items-center`}>
                    <Text className={`${isLight ? 'text-zinc-600' : 'text-zinc-500'}  text-[10px] uppercase tracking-wider font-bold mb-4`}>Scan QR Code</Text>
                    {otpauthUri ? (
                ...
                <View className={`${isLight ? 'bg-white' : 'bg-zinc-900'}  p-4 rounded-2xl border border-zinc-800`}>
                  <View className="flex-row items-center gap-2 mb-3">
                    <View className="w-7 h-7 bg-blue-600 rounded-lg items-center justify-center">
                      <Text className={`${isLight ? 'text-zinc-900' : 'text-white'}  font-bold text-xs`}>3</Text>
                    </View>
                    <Text className={`${isLight ? 'text-zinc-900' : 'text-white'}  font-semibold text-sm`}>Verify Setup</Text>
                  </View>
                  <Text className={`${isLight ? 'text-zinc-500' : 'text-zinc-400'}  text-xs leading-5 mb-3`}>
                    Enter the 6-digit code currently shown in your authenticator app to confirm setup:
                  </Text>

                  <View className={`${isLight ? 'bg-zinc-50' : 'bg-zinc-950'} flex-row items-center  rounded-xl px-4 py-3 border border-zinc-800`}>
                    <Key color="#3b82f6" size={18} />
                    <TextInput
                      className={`${isLight ? 'text-zinc-900' : 'text-white'} flex-1  ml-3 text-lg font-bold tracking-[0.3em] text-center`}
                      placeholder="000000"
                  ...
                  <TouchableOpacity
                    ...
                    disabled={loading}
                  >
                    <Text className={`${isLight ? 'text-zinc-900' : 'text-white'}  font-bold text-sm uppercase tracking-wide`}>
                ...
                >
                  <Text className={`${isLight ? 'text-zinc-600' : 'text-zinc-500'}  text-sm font-medium`}>Cancel Setup</Text>
                </TouchableOpacity>
              </View>
            ) : (
              <View className="gap-4">
                ...
                {is2FAEnabled ? (
                  <>
                    <TouchableOpacity
                      ...
                      className={`${isLight ? 'bg-white' : 'bg-zinc-900'}  p-4 rounded-2xl border border-zinc-800 flex-row items-center gap-3`}
                    >
                      <Key color="#3b82f6" size={20} />
                      <View className="flex-1">
                        <Text className={`${isLight ? 'text-zinc-900' : 'text-white'}  font-semibold text-sm`}>Reset ZestyAuth</Text>
                        <Text className={`${isLight ? 'text-zinc-600' : 'text-zinc-500'}  text-xs mt-0.5`}>Generate a new secret key</Text>
                      </View>
                    </TouchableOpacity>
                    ...
                    >
                      ...
                      <View className="flex-1">
                        ...
                        <Text className={`${isLight ? 'text-zinc-600' : 'text-zinc-500'}  text-xs mt-0.5`}>Remove two-factor authentication</Text>
                      </View>
                    </TouchableOpacity>
                  </>
                ) : (
                  ...
                  >
                    ...
                    <View className="flex-1">
                      ...
                      <Text className={`${isLight ? 'text-zinc-500' : 'text-zinc-400'}  text-xs mt-1 leading-4`}>
                ...
                {/* How it works */}
                <View className={`${isLight ? 'bg-white' : 'bg-zinc-900'} /50 p-4 rounded-2xl border border-zinc-800/50 mt-2`}>
                  <Text className={`${isLight ? 'text-zinc-500' : 'text-zinc-400'}  text-xs font-bold uppercase tracking-wider mb-3`}>How ZestyAuth Works</Text>
                  <Text className={`${isLight ? 'text-zinc-600' : 'text-zinc-500'}  text-xs leading-5`}>
